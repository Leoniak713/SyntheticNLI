...
class Triplet:
    ...
    def parsed(self):
        if self.property_is_functional():
            ...
            return {(self.subject.name, self._property.name, _object.name) for _object in self._object}
    ...
    def __str__(self):
        return ' '.join([self.subject.name, self._property.name]+[o.name for o in self.get_object_entities()])
...
class WorldGenerator(ABC):
    ...
    def get_property_domain_range_map(self):
        filled_properties = self._get_property_domains_and_ranges()
        return {_property: (domains, ranges) for _property, domains, ranges in filled_properties}
...
class PropertyBasedGenerator(WorldGenerator):
    ...
    def modify_triplets(self, inferred_triplets):
        ...
        for triplet in inferred_triplets:
            ...
            valiable_subjects = [i for i in self.ontology.get_instances_of(domains[0]) if i != triplet.subject]
            if len(valiable_subjects) > 0:
                subject = random.choice(valiable_subjects)
            else:
                subject = domains[0]()
    ...
    def generate_world(self, num_properties):
        ...
        inferred_triplets = parse_world(post_inference_triplets) - parsed_initial_triplets
